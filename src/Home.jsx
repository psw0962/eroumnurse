...
const SERVICES = [
  {
    icon: "💉",
    title: "전문 간호 서비스",
    desc: "욕창, 상처 드레싱, 콧줄, 소변줄 관리 등 가정에서 필요한 전문 간호 처치를 면허 간호사/간호조무사가 직접 제공합니다.",
  },
  {
    icon: "💊",
    title: "투약 관리",
    desc: "복잡한 약물 복용 스케줄을 관리하고, 부작용 모니터링 및 의사와의 연계를 도와드립니다.",
  },
  {
    icon: "🩺",
    title: "건강 상태 모니터링",
    desc: "활력징후 측정, 혈당·혈압 관리 등 만성질환 어르신의 건강 변화를 체계적으로 관찰합니다.",
  },
  {
    icon: "🧠",
    title: "인지기능 관리",
    desc: "치매·인지 저하 어르신을 위한 전문적인 인지 자극 프로그램을 간호사/간호조무사가 직접 설계·운영합니다.",
  },
  {
    icon: "🏥",
    title: "1:1 맞춤 방문재활",
    desc: "뇌졸증, 파킨슨, 수술 후 가정에서 맞춤 재활을 밀착 지원하여 재입원을 예방합니다.",
  },
  {
    icon: "👨‍👩‍👧",
    title: "가족 교육 및 상담",
    desc: "보호자가 가정에서 올바른 돌봄을 실천할 수 있도록 간호사/간호조무사가 직접 교육하고 상담합니다.",
  },
];
...
const WHY_US = [
  {
    icon: "👩‍⚕️",
    title: "100% 면허 간호사 운영",
    desc: "요양보호사가 아닌 국가면허 간호사 또는 간호조무사가 직접 방문합니다.",
  },
  {
    icon: "📋",
    title: "개인 맞춤 간호 계획",
    desc: "어르신별 건강 상태에 맞는 간호 계획을 수립합니다.",
  },
  {
    icon: "🔗",
    title: "의료기관 연계 시스템",
    desc: "병원·의원과의 긴밀한 연계로 신속한 의료 대응이 가능합니다.",
  },
  {
    icon: "📊",
    title: "체계적 기록 관리",
    desc: "매 방문 간호 기록을 작성하여 가족과 투명하게 공유합니다.",
  },
];
...
const STEPS = [
  ...
  },
  {
    num: "02",
    title: "간호사 방문 상담",
    desc: "면허 간호사/간호조무사가 직접 방문하여 어르신 건강 상태를 전문적으로 평가합니다.",
  },
  {
    num: "03",
    title: "장기요양 등급 신청",
    desc: "공단에 등급 신청을 도와드립니다.",
  },
  {
    num: "04",
    title: "담당 간호사 매칭",
    desc: "어르신의 질환과 성향에 맞는 전담 간호사를 신중하게 배정합니다.",
  },
  {
    num: "05",
    title: "방문 간호 시작",
    ...
    <div className="font-sans bg-bora-50 text-bora-900 min-h-screen">
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled || menuOpen
            ? "bg-bora-50/98 backdrop-blur-md border-b border-bora-200 shadow-sm"
            : "bg-transparent"
        }`}
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
            ...
            {[
              ...
              <button
                ...
                className={`text-sm relative pb-0.5 transition-colors duration-200 after:absolute after:bottom-0 after:left-0 after:h-0.5 after:bg-bora-600 after:rounded-full after:transition-all after:duration-300 ${
                  activeNav === id
                    ? "text-bora-600 after:w-full"
                    : "text-bora-500 hover:text-bora-600 after:w-0 hover:after:w-full"
                }`}
              >
                {label}
              </button>
          ...
          >
            <span
              className={`block w-6 h-0.5 bg-bora-600 rounded-full transition-all duration-300 origin-center ${menuOpen ? "rotate-45 translate-y-2" : ""}`}
            />
            <span
              className={`block w-6 h-0.5 bg-bora-600 rounded-full transition-all duration-300 ${menuOpen ? "opacity-0 scale-x-0" : ""}`}
            />
            <span
              className={`block w-6 h-0.5 bg-bora-600 rounded-full transition-all duration-300 origin-center ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center">
            <div className="text-center md:text-left order-2 md:order-1">
              ...
              <p className="text-bora-500 leading-relaxed mb-8 text-sm sm:text-[0.97rem]">
                이로움방문간호센터는{" "}
                <strong className="text-bora-600">
                  국가면허 간호사/간호조무사
                </strong>
                가 직접 운영·방문하는 방문간호센터입니다. 어르신의 건강을 의료
                전문가의 눈으로 세심하게 살펴드립니다. 아산병원 출신 의학박사가
                교육하는 수련과정을 수료한 선생님들이 방문하여 집에서도 병원급
                재활/간호 서비스를 받으실 수 있습니다.
              </p>
            ...
            <div className="animate-float order-1 md:order-2">
              <div className="relative bg-linear-to-br from-bora-100 to-bora-200 rounded-3xl p-8 sm:p-12 text-center border-2 border-bora-200 overflow-hidden">
                ...
                <img
                  src={logo}
                  alt="logo"
                  className="w-28 h-28 sm:w-40 sm:h-40 object-contain mx-auto mb-4"
                />
      ...
      <section
        id="services"
        className="py-16 sm:py-24 bg-bora-100 px-4 sm:px-6"
      >
        <div className="max-w-6xl mx-auto">
          <FadeIn>
            <div className="text-center mb-10 sm:mb-14">
        ...
        <div className="max-w-2xl mx-auto">
          <FadeIn>
            <div className="text-center mb-10 sm:mb-14">
              ...
              <h2 className="text-3xl sm:text-4xl font-bold text-bora-900">
                이용 절차
              </h2>
            </div>
          </FadeIn>
              ...
              {STEPS.map((step, i) => (
                <FadeIn key={step.num} delay={i * 0.1}>
                  <div className="flex gap-4 sm:gap-5 items-start relative z-10">
                    <div
                      className={`shrink-0 w-[52px] h-[52px] rounded-full flex items-center justify-center font-bold text-sm ${
                        i === 0
                          ? "bg-linear-to-br from-bora-600 to-bora-500 text-white shadow-bora"
                          : "bg-white border-2 border-bora-200 text-bora-600"
                      }`}
                    >
                      {step.num}
                    </div>
          ...
          <FadeIn>
            <div className="text-center mb-4">
              ...
              <p className="text-bora-500 text-sm">
                장기요양보험 적용 시 본인부담금은 소득 수준에 따라 달라집니다.
              </p>
            </div>
          </FadeIn>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mt-10 sm:mt-12 items-center">
            {PLANS.map((plan, i) => (
              <FadeIn key={plan.name} delay={i * 0.1}>
                <div
                  className={`relative rounded-3xl p-7 sm:p-9 border-2 transition-all duration-300 ${
                    plan.highlight
                ...
                >
                  ...
                  <div
                    className={`text-sm mb-5 ${plan.highlight ? "text-white/70" : "text-bora-400"}`}
                  >
                    {plan.hours}
                  </div>
                  <div
                    className={`text-sm font-semibold mb-5 pb-5 border-b ${plan.highlight ? "border-white/20 text-white" : "border-bora-200 text-bora-700"}`}
                  >
                    본인부담금 산정 후 안내
                  </div>
                  <ul className="space-y-3 mb-8">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-2 text-sm">
                        <span
                          className={`font-bold mt-0.5 ${plan.highlight ? "text-lavender" : "text-bora-600"}`}
                        >
                          ✓
                        </span>
                        {f}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
            <p className="text-xs text-white/30">
              © 2026 이로움방문간호센터. All rights reserved.
            </p>
